...
De Novo generation of molecules for 5-HT2A receptor using Reinvent 4 and Chemprop as scoring function.
"""
...
# ////////////////////////////////////////////////////////////////////////////
CHEMPROP_PYTHON_PATH = "/home/sdageri/miniconda3/envs/chemprop/bin/python"  #this path needs to be updated based on the user's system and it's necessary for the scorer.py script to run
DEVICE = "cuda"
BATCH_SIZE = 256
S1_STEPS = 500
TL_EPOCHS = 20
...
def log(msg):
    """Prints immediately to standard output for real-time tracking."""
    print(f"[run staged learning pipeline] {msg}", flush=True)

def run_step(name, cmd, check_file=None):
    """
    Executes a shell command and verifies its completion.
    
    Args:
        name (str): Display name for the step in the logs.
        cmd (list): Subprocess command arguments.
        check_file (str or Path, optional): File expected to be generated by the command.
                                            If not found upon completion, the pipeline stops.
    Returns:
        CompletedProcess instance containing execution details.
    """
    log(f"Starting: {name}")
...
def prepare_smiles():
    """
    Reads the initial smiles dataset, cleans it, and performs an 80/20 
    train/validation split required for the Transfer Learning phase.
    
    Returns:
        tuple: Paths to the newly generated train and validation .smi files.
    """
...
def stage1_toml(prior, out):
    """
    Generates the TOML configuration for Stage 1 Reinforcement Learning.
    Focuses on scoring physicochemical properties (QED, TPSA) and penalizing specific structural alerts.
    """
    return f"""\
...
def tl_toml(input_model, output_model, train_smiles, val_smiles, out):
    """
    Generates the TOML configuration for the Transfer Learning phase.
    Uses the specified dataset to adapt the model to the target chemical space.
    """
    return f"""\
...
def stage2_toml(prior, agent, scorer, chemprop_py, out):
    """
    Generates the TOML configuration for Stage 2 Reinforcement Learning.
    Incorporates an external chemprop model(MPNN) to guide the generation
    towards high-affinity 5-HT2A compounds, balancing with QED and TPSA.
    """
    return f"""\
run_type = "staged_learning"
device   = "{DEVICE}"
...
def main():
    """
    Orchestrates the overall pipeline workflow:
    1. Prepares the SMILES data.
    2. Runs Stage 1 Reinforcement Learning to pre-train on general properties.
    3. Runs Transfer Learning to narrow the chemical focus.
    4. Runs Stage 2 Reinforcement Learning to optimize for the 5-HT2A target using Chemprop.
    """
